...
class DATABASE():
    ...
    def getFulldataTeknikServisTable(self):
        self.cursor.execute("SELECT * FROM tbl_TeknikServis")
        rows = self.cursor.fetchall()
        return rows
    
    def getFulldataGiderTable(self):
        self.cursor.execute("SELECT * FROM tbl_Gider")
        rows = self.cursor.fetchall()
        return rows
